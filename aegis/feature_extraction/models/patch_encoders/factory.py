...
from aegis.feature_extraction.models.patch_encoders.utils.transform_utils import (
    get_eval_transforms,
)
...
class BasePatchEncoder(torch.nn.Module):
    ...
    def __init__(self, weights_path: Optional[str] = None, **build_kwargs):
        ...
        self.model, self.eval_transforms, self.precision, self.embedding_dim = (
    ...
    def ensure_has_internet(self, enc_name):
        if not BasePatchEncoder._has_internet:
            raise FileNotFoundError(
                ...
                f"and place it in the model registry in:\n`aegis/feature_extraction/models/patch_encoders/checkpoints.json`"
            )
...
class MuskInferenceEncoder(BasePatchEncoder):
    ...
    def _build(
        self, inference_aug=False, with_proj=False, out_norm=False, return_global=True
    ):
        """
        Args:
        ...
        if weights_path:
            raise NotImplementedError(
                "MUSK doesn't support local model loading. PR welcome!"
            )
        else:
        ...
        if weights_path:
            try:
                model, eval_transform = create_model_from_pretrained(
                    "conch_ViT-B-16", checkpoint_path=weights_path
                )
            except Exception:
                traceback.print_exc()
                raise Exception(
            ...
            try:
                ...
                traceback.print_exc()
                raise Exception(
                    "Failed to download CONCH v1 model, make sure that you were granted access and that you correctly registered your token"
                )
...
class CTransPathInferenceEncoder(BasePatchEncoder):
    ...
    def _build(self):
        ...
        try:
            from aegis.feature_extraction.models.patch_encoders.zoo.ctranspath.ctran import (
                ctranspath,
            )
        except Exception:
            traceback.print_exc()
        ...
        if not weights_path:
            ...
            try:
                ...
                traceback.print_exc()
                raise Exception(
                    "Failed to download CTransPath model, make sure that you were granted access and that you correctly registered your token"
                )
        ...
        if weights_path:
            try:
                ...
                model = ViTModel.from_pretrained(
                    model_dir, add_pooling_layer=False, local_files_only=True
                )
            except Exception:
                traceback.print_exc()
                raise Exception(
            ...
            try:
                ...
                traceback.print_exc()
                raise Exception(
                    "Failed to download Phikon model, make sure that you were granted access and that you correctly registered your token"
                )
...
class HibouLInferenceEncoder(BasePatchEncoder):
    ...
    def _build(self):
        ...
        if weights_path:
            raise NotImplementedError(
                "Hibou-L doesn't support local model loading. PR welcome!"
            )
        else:
            self.ensure_has_internet(self.enc_name)
            try:
                ...
                traceback.print_exc()
                raise Exception(
                    "Failed to download Hibou-L model, make sure that you were granted access and that you correctly registered your token"
                )
    ...
    def _build(self):
        ...
        if weights_path:
            try:
                ...
                model = timm.create_model(
                    "vit_small_patch8_224", checkpoint_path=weights_path, **timm_kwargs
                )
            except:
                traceback.print_exc()
                raise Exception(
            ...
            try:
                model = timm.create_model(
                    "hf-hub:1aurent/vit_small_patch8_224.lunit_dino", pretrained=True
                )
            except:
                traceback.print_exc()
                raise Exception(
                    "Failed to download Lunit S8 model, make sure that you were granted access and that you correctly registered your token."
                )
...
class UNIv2InferenceEncoder(BasePatchEncoder):
    ...
    def _build(self):
        ...
        if weights_path:
            try:
                model = timm.create_model(
                    model_name="vit_giant_patch14_224", pretrained=False, **timm_kwargs
                )
                model.load_state_dict(
                    torch.load(weights_path, map_location="cpu"), strict=True
                )
            ...
            try:
                ...
                traceback.print_exc()
                raise Exception(
                    "Failed to download UNI v2 model, make sure that you were granted access and that you correctly registered your token"
                )
    ...
    def _build(
        ...
        if weights_path:
            try:
                ...
                model = timm.create_model(
                    "vit_giant_patch14_dinov2", pretrained=False, **timm_kwargs
                )
                model.load_state_dict(
                    torch.load(weights_path, map_location="cpu"), strict=True
                )
            ...
            try:
                ...
                traceback.print_exc()
                raise Exception(
                    "Failed to download GigaPath model, make sure that you were granted access and that you correctly registered your token"
                )
    ...
    def _build(
        ...
        if weights_path:
            try:
                ...
                model.load_state_dict(
                    state_dict=torch.load(weights_path, map_location="cpu"), strict=True
                )
            except:
                traceback.print_exc()
                raise Exception(
            ...
            try:
                ...
                traceback.print_exc()
                raise Exception(
                    "Failed to download Virchow model, make sure that you were granted access and that you correctly registered your token"
                )
    ...
    def _build(
        ...
        if weights_path:
            ...
            try:
                ...
                traceback.print_exc()
                raise Exception(
                    "Failed to download Virchow-2 model, make sure that you were granted access and that you correctly registered your token"
                )
    ...
    def _build(self, timm_kwargs={"init_values": 1e-5, "dynamic_img_size": False}):
        ...
        if weights_path:
            try:
                ...
                model = timm.create_model(
                    "vit_giant_patch14_reg4_dinov2", **timm_kwargs
                )
                model.load_state_dict(
                    torch.load(weights_path, map_location="cpu"), strict=True
                )
            ...
            try:
                ...
                traceback.print_exc()
                raise Exception(
                    "Failed to download HOptimus-0 model, make sure that you were granted access and that you correctly registered your token"
                )
    ...
    def _build(
        ...
        if weights_path:
            ...
            try:
                ...
                traceback.print_exc()
                raise Exception(
                    "Failed to download HOptimus-1 model, make sure that you were granted access and that you correctly registered your token"
                )
    ...
    def _build(self):
        ...
        if weights_path:
            ...
            try:
                ...
                traceback.print_exc()
                raise Exception(
                    "Failed to download Phikon v2 model, make sure that you were granted access and that you correctly registered your token"
                )
...
class Conchv15InferenceEncoder(BasePatchEncoder):
    ...
    def _build(self, img_size=448):
        from aegis.feature_extraction.models.patch_encoders.zoo.conchv1_5.conchv1_5 import (
        ...
        if weights_path:
            ...
            try:
                ...
                traceback.print_exc()
                raise Exception(
                    "Failed to download CONCH v1.5 model, make sure that you were granted access and that you correctly registered your token"
                )
