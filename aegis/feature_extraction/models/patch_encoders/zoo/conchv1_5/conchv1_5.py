...
class Block(nn.Module):
    def __init__(
        ...
        qkv_bias=False,
        qk_norm=False,
        proj_drop=0.0,  # proj -> proj_drop renamed
        attn_drop=0.0,
        init_values=None,
        drop_path=0.0,
        act_layer=nn.GELU,
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        drop_rate: float = 0.0,
        pos_drop_rate: float = 0.0,  # new
        patch_drop_rate: float = 0.0,  # new
        proj_drop_rate: float = 0.0,  # renamed
        attn_drop_rate: float = 0.0,  # same
        drop_path_rate: float = 0.0,  # same
        weight_init: str = "",
...
def create_model_from_pretrained(
    checkpoint_path: str, cache_dir: Optional[str] = None, img_size: int = 448
):
    ...
    state_dict["trunk.pos_embed"] = resize_pos_embed(
        model.trunk, state_dict["trunk.pos_embed"], verbose=False
    )
