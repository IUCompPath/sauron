    ...
    def forward(self, x, return_attn=False):
        ...
        if self.crmsa_mlp:
            logits = self.phi(x_regions).transpose(
                1, 2
            )  # W*B, sW, region_size*region_size
        else:
            logits = torch.einsum("w p c, c n -> w p n", x_regions, self.phi).transpose(
                1, 2
        ...
        attn_regions = (
            torch.einsum("w p c, w n p -> w n p c", x_regions, combine_weights)
            .sum(dim=-2)
            .transpose(0, 1)
        )  # sW, nW, C
