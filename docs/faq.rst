...
.. dropdown:: **How do I extract embeddings from legacy CLAM coordinates?**
   ...
   .. code-block:: bash
      ...
      aegis extract_patches --task feat --wsi_dir wsis --job_dir legacy_dir --coords_dir extracted_coords --patch_encoder uni_v1
...
.. dropdown:: **I want to skip patches on holes.**
   ...
   By default, AEGIS includes all tissue patches (including holes). Use `--remove_holes` to exclude them. Not recommended, as "holes" often help define the tissue microenvironment.
...
.. dropdown:: **I work on a cluster without Internet access. How can I use models offline?**
   ...
   You can use local checkpoint files by editing the model registry files in AEGIS. This allows you to cache or pre-download all necessary models for both segmentation and patch encoding.

   **1. Segmentation Models**

   Update the segmentation model registry at:
   `aegis/segmentation_models/local_ckpts.json`
   ...
   .. code-block:: json
      ...
      {
        "hest": "./ckpts/aegis/deeplabv3_seg_v4.ckpt",
        "grandqc": "./ckpts/aegis/Tissue_Detection_MPP10.pth",
        "grandqc_artifact": "./ckpts/aegis/GrandQC_MPP1_state_dict.pth"
      }

   **2. Patch Encoder Models**

   Update the patch encoder model registry at:
   `aegis/patch_encoder_models/local_ckpts.json`
   ...
   **3. Alternative Option**
   ...
   You can also directly pass a local checkpoint path at runtime using the `--patch_encoder_ckpt_path` argument in `aegis extract_patches`.

   **4. Optional: Pre-download All Models in Advance**

   If you'd like to automatically download all model weights in advance (e.g., from a connected machine), use the following:

   .. code-block:: bash

      XDG_CACHE_HOME="<YOUR_CACHE_DIR>" HF_TOKEN="<YOUR_HUGGINGFACE_TOKEN>" python predownload_weights.py

   This will fetch all segmentation, patch encoder, and slide encoder weights supported in AEGIS.

   To run downstream tasks using the cached models:

   .. code-block:: bash

      XDG_CACHE_HOME="<YOUR_CACHE_DIR>" aegis extract_patches ...
      XDG_CACHE_HOME="<YOUR_CACHE_DIR>" aegis train_mil ...

   Example `predownload_weights.py` script (can be adapted based on needs):

   .. code-block:: python

      from aegis.segmentation_models import segmentation_model_factory
      from aegis.patch_encoder_models.load import encoder_factory as patch_encoder_model_factory
      from aegis.slide_encoder_models.load import encoder_factory as slide_encoder_model_factory
